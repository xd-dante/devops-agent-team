...
| Finding | Hand to |
|---------|---------|
| Commit landed but the deployment did not move | `argocd-agent` |
| Chart mechanics — templates, values shape | `helm-agent` |
| The integration annotation or Application spec | `terraform-agent` |
| Confirm the workload after a promotion | `kubernetes-agent` (read-only, direct) |
