...
| Finding | Hand to |
|---------|---------|
| CI fails on a chart or manifest render | `helm-agent` |
| CI fails on infrastructure validation or formatting | `terraform-agent` |
| The PR needs linking onto a ticket | `jira-agent` |
| Anything outside delivery | back to `devops-orchestrator` |
