...
|-------|--------------------|
| `kubernetes-agent` | What does the workload say — restarts, probes, OOM, scheduling, events? |
| `argocd-agent` | Is desired state what we think — synced, drifted, pinned revision, last sync result? |
| `aws-investigator-agent` | Is a dependency below the cluster failing — database, secrets, IAM, quotas? |
| `kargo-agent` | Only if the symptom is version-shaped ("wrong version", "deploy never arrived") |
...
Recommended fix (not applied):
  - <change>  → route: terraform-agent (<repo>, targeted plan)
  - <change>  → route: helm-agent (<chart>/values-<env>.yaml)
```
