...
| Found? | Do |
|--------|----|
| Yes | `HANDOFF → jira-agent` with `actions/read-ticket.md` |
| No | `HANDOFF → jira-agent` with `actions/create-ticket.md`, then read it back |
...
## Step 3 — Worktree (`github-agent`)

`HANDOFF → github-agent` with `actions/setup-worktree.md`. It anchors to the
...
| Work | Agent |
|------|-------|
| Terraform resources, variables, environment values | `terraform-agent` |
| Chart templates and values | `helm-agent` |
| GitOps Application wiring | `argocd-agent` |
| Promotion pipeline config | `kargo-agent` |
| Verifying live state before or after | `kubernetes-agent`, `aws-investigator-agent` |

Sequence anything with a dependency. Never two mutating agents in the same
files at once.

## Step 5 — Commit and PR (`github-agent`)

`HANDOFF → github-agent` with `actions/open-pull-request.md`. It handles
...
`HANDOFF → jira-agent` with `actions/update-ticket.md` to link the PR onto
the ticket.
...
| Trusting the remote's advertised default branch | `github-agent` probes it |
