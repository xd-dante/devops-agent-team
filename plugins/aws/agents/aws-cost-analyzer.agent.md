...
| Finding | Hand to |
|---------|---------|
| Any accepted change | `terraform-engineer` (repo and file named) |
| Oversized pod requests driving node cost | `helm-engineer` |
| Observed usage needed to size something | `kubernetes-investigator` (read-only, direct) |
| Tracking the findings | `ticket-analyst` — one ticket per actionable item |
...
Owner: terraform-engineer → <infra repo> monitoring values
