...
| Finding | Hand to |
|---------|---------|
| The value is computed, not static | `terraform-engineer` |
| Renders correctly but live differs | `argocd-analyst` (sync state or pin) |
| The image version itself is wrong | `kargo-promoter` |
| Need observed usage to size resources | `kubernetes-investigator` (read-only, direct) |
