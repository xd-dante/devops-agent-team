...
| Finding | Hand to |
|---------|---------|
| Resources, probes, env vars, replicas | `helm-agent` |
| Wrong image version | `kargo-agent` |
| Not synced, drifted, pinned revision | `argocd-agent` |
| Node pools, add-ons, identity bindings, network rules | `terraform-agent` |
| Managed database, network or quota behaviour | `aws-investigator-agent` |

Mutating handoffs go back through `devops-orchestrator`. You may answer a
...
Owner:      helm-agent → charts/api/values-staging.yaml
Nothing was mutated.
```
