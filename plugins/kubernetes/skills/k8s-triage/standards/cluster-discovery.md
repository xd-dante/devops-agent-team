...
| Finding | Owner |
|---------|-------|
| Resources/limits, probes, env vars, replicas | `helm-engineer` → chart values |
| Wrong image version | `kargo-promoter` → promotion |
| Not synced, drifted, pinned revision | `argocd-analyst` |
| Node pools, autoscaling, cluster add-ons | `terraform-engineer` |
| Network policy, identity bindings, secrets wiring | `terraform-engineer` |
| Managed database, network, quota behaviour | `aws-investigator` |
