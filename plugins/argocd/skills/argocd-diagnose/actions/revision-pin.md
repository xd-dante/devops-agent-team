...
| Situation | Fix | Owner |
|-----------|-----|-------|
| The pin should move to the new commit or version | Update it in the module call or environment values, then a targeted apply | `terraform-engineer` |
| The app should track the branch instead | Change it to the branch — **with the reason stated**, since pins usually exist deliberately | `terraform-engineer` |
| Promotion tooling manages the pin | Promote instead of editing code | `kargo-promoter` |
...
Owner:           terraform-engineer (or kargo-promoter if promotion manages it)
