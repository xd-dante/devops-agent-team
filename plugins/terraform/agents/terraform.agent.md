...
| Finding | Hand to |
|---------|---------|
| The value is a chart value, not an infrastructure one | `helm-agent` |
| Need to confirm the live effect in a cluster | `kubernetes-agent` (read-only) |
| Need to confirm a cloud resource's real state | `aws-investigator-agent` (read-only) |
| The deployment is not picking the change up | `argocd-agent` |
