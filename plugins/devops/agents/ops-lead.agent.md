...
| the user says remember / keep in mind / forget that | `remember` skill (`memory` plugin) |
...
| Agent | Owns | Mutates? |
|-------|------|----------|
| `ticket-analyst` | Ticket context, creation, comments, transitions | tracker only |
| `delivery-engineer` | Worktrees, branches, commits, PRs, reviews, CI | repo only |
| `terraform-engineer` | Plans, applies, modules, variables, state | gated |
| `helm-engineer` | Chart templates, values, library charts | files only |
| `kubernetes-investigator` | Live cluster investigation | **read-only** |
| `argocd-analyst` | Application health, drift, revisions, sync | gated |
| `kargo-promoter` | Freight, Stages, promotions | gated |
| `aws-investigator` | Cloud resource behaviour | **read-only** |
| `aws-cost-analyzer` | Spend, rightsizing, savings | **read-only** |
...
```
Routing to kubernetes-investigator (pod-level triage) and argocd-analyst (sync state), in parallel.

HANDOFF → kubernetes-investigator
